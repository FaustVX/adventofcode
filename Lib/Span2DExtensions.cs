...
public static partial class Span2DExtensions
{
    extension<T>(Span2D<T> span) where T : IEquatable<T>?
    {
        ...
        public (int row, int column) IndexOfAny(SearchValues<T> values)
        => IndexOfAny((ReadOnlySpan2D<T>)span, values);

        /// <inheritdoc cref="IndexOfAnyExcept{T}(ReadOnlySpan2D{T}, SearchValues{T})"/>
        public (int row, int column) IndexOfAnyExcept(SearchValues<T> values)
    ...
    {
        public ReadOnlySpan2DBlockEnumerator<T> EnumerateBlocks(int width, int height)
