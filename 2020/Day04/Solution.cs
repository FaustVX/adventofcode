...
public class Solution : Solver
{
    ...
    public object PartOne(string input)
        => PartOne(GetPassports(input));

    public object PartTwo(string input)
        => PartTwo(GetPassports(input));

    static int PartOne(Dictionary<string, string>[] passports)
        => passports.Count(p => p.Count == 8 || (p.Count == 7 && !p.ContainsKey("cid")));

    static int PartTwo(Dictionary<string, string>[] passports)
    {
